...
use mcp_protocol_sdk::{
    protocol::{
        types::{JsonRpcRequest, ClientCapabilities, Implementation, LATEST_PROTOCOL_VERSION},
...
async fn test_initialization_fix() {
    ...
    // Verify the response
    assert!(response.is_ok(), "Initialize request should succeed: {:?}", response.err());
    ...
    if let Some(result) = response.result {
        assert!(result.get("protocolVersion").is_some(), "Response should contain protocolVersion");
        assert!(result.get("capabilities").is_some(), "Response should contain capabilities");
        assert!(result.get("serverInfo").is_some(), "Response should contain serverInfo");
    ...
    println!("✅ Initialization test passed! Server correctly handles initialize method.");
}
...
async fn test_method_not_found_for_unknown_method() {
    ...
    match response {
        ...
        Err(error) => {
            ...
            assert!(error.to_string().contains("unknown_method"), "Error should mention the unknown method");
        }
    }
    
    println!("✅ Unknown method test passed! Server handles unknown methods appropriately.");
}
...
async fn test_transport_integration() {
    ...
    let request_handler: mcp_protocol_sdk::transport::traits::ServerRequestHandler = {
    ...
    println!("✅ Transport integration test passed! Transport can now properly delegate requests to server.");
}
...
async fn test_complete_server_startup() {
    ...
    println!("✅ Complete server startup test passed! Server is configured correctly for startup.");
}
