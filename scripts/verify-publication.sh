...
echo "🔍 MCP Protocol SDK - Pre-Publication Verification"
echo "=================================================="
...
if command -v gh &> /dev/null; then
    PAGES_URL=$(gh api repos/mcp-rust/mcp-protocol-sdk/pages --jq '.html_url' 2>/dev/null || echo "")
    if [ -n "$PAGES_URL" ]; then
...
echo ""
echo -e "${BLUE}📦 Ready to publish to crates.io:${NC}"
