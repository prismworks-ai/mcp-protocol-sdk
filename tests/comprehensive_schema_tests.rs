...
use mcp_protocol_sdk::{
    protocol::{types::*, messages::*, validation::*},
};
use serde_json::json;
...
mod protocol_constants {
    ...
    fn test_method_constants_complete_coverage() {
        ...
        assert_eq!(methods::TOOLS_LIST_CHANGED, "notifications/tools/list_changed");
        ...
        assert_eq!(methods::RESOURCES_UPDATED, "notifications/resources/updated");
        assert_eq!(methods::RESOURCES_LIST_CHANGED, "notifications/resources/list_changed");
        ...
        assert_eq!(methods::PROMPTS_LIST_CHANGED, "notifications/prompts/list_changed");
...
mod content_types {
    ...
    fn test_audio_content_new_in_2025() {
        ...
        let audio_formats = vec![
            "audio/wav", "audio/mp3", "audio/ogg", "audio/aac", "audio/flac"
        ];
        
        for format in audio_formats {
            let content = Content::audio("data", format);
            assert!(validate_content(&content).is_ok(), "Failed for format: {}", format);
        }
...
mod annotations_system {
    ...
    fn test_annotation_creation_and_builders() {
        let annotations = Annotations::new()
            .read_only()
            .for_audience(vec![AnnotationAudience::User, AnnotationAudience::Developer])
            .with_danger_level(DangerLevel::Safe);
    ...
    fn test_destructive_tool_annotations() {
        let destructive_annotations = Annotations::new()
...
mod tool_system {
    ...
    fn test_tool_creation_with_annotations() {
        let tool = Tool::new("file_reader", "Safely read files")
            .with_annotations(
                Annotations::new()
                    .read_only()
                    .for_audience(vec![AnnotationAudience::User])
    ...
    fn test_call_tool_result_with_metadata() {
        ...
        let result_with_meta = CallToolResult::success(vec![Content::text("Done")])
            .with_metadata(meta);
...
mod new_features_2025 {
    ...
    fn test_enhanced_progress_notifications() {
        ...
        assert_eq!(progress.message, Some("Processing files... 75% complete".to_string()));
