...
for workflow in "${WORKFLOWS[@]}"; do
    if [ -f ".github/workflows/$workflow" ]; then
        print_status "✅" "Workflow file: $workflow"
    else
...
if [ -f "codecov.yml" ]; then
    if grep -q "token:" codecov.yml && ! grep -q "# Token configured in GitHub Secrets" codecov.yml; then
        print_status "⚠️" "codecov.yml contains hardcoded token (security risk)"
        echo "    Run: Remove hardcoded token from codecov.yml"
    else
        print_status "✅" "codecov.yml properly configured"
    fi
else
...
for workflow in .github/workflows/*.yml; do
    if [ -f "$workflow" ]; then
        if python3 -c "import yaml; yaml.safe_load(open('$workflow'))" 2>/dev/null; then
            print_status "✅" "$(basename "$workflow"): Valid YAML"
        else
...
print_status "⚠️" "CODECOV_TOKEN: Must be set in GitHub Secrets"
print_status "⚠️" "CARGO_REGISTRY_TOKEN: Must be set in GitHub Secrets"
...
if [ -f "codecov.yml" ] && ! grep -q "token:" codecov.yml; then
    print_status "✅" "Configuration files are secure"
else
    print_status "⚠️" "Some configuration files need attention"
fi

if [ -d ".github/workflows" ] && [ "$(ls -1 .github/workflows/*.yml 2>/dev/null | wc -l)" -eq 6 ]; then
    print_status "✅" "All workflow files present"
else
    print_status "⚠️" "Some workflow files may be missing"
fi

print_status "⚠️" "GitHub Secrets need manual setup"
...
echo
echo "💡 For detailed instructions, see the Environment Setup Analysis document"
