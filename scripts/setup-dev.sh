...
# Install cargo-tarpaulin for code coverage (Linux/macOS only)
if [[ "$OSTYPE" == "linux-gnu"* ]] || [[ "$OSTYPE" == "darwin"* ]]; then
    if ! command -v cargo-tarpaulin &> /dev/null; then
        ...
        if cargo install cargo-tarpaulin; then
            ...
            print_warning "Failed to install cargo-tarpaulin (not critical)"
        fi
    else
        print_success "cargo-tarpaulin is already installed"
    fi
else
    print_warning "cargo-tarpaulin is not supported on Windows (coverage will be skipped)"
fi
...
print_status "  • Code coverage:      cargo tarpaulin --all-features"
echo ""
